...
export class Interaction<T extends ActionName | null = ActionName> {
    ...
    pointerDown(
        ...
        const pointerIndex = this.updatePointer(
            pointer,
            event,
            eventTarget,
            true
    ...
    pointerUp(
        ...
        this._scopeFire(
            `interactions:${type}` as "interactions:up" | "interactions:cancel",
            {
                pointer,
                pointerIndex,
                pointerInfo: this.pointers[pointerIndex],
    ...
    getPointerIndex(pointer: any) {
        ...
        return this.pointerType === "mouse" || this.pointerType === "pen"
            ...
            : arr.findIndex(
                  this.pointers,
                  (curPointer) => curPointer.id === pointerId
              );
    }
    ...
    updatePointer(
        ...
        down =
            down === false ? false : down || /(down|start)$/i.test(event.type);
    ...
    _doPhase<P extends EventPhase>(
        signalArg: Omit<DoPhaseArg<T, P>, "iEvent"> & {
            iEvent?: InteractEvent<T, P>;
        }
    ) {
        ...
        const beforeResult = this._scopeFire(
            `interactions:before-action-${phase}` as any,
            signalArg
        );
        ...
        const iEvent = (signalArg.iEvent = this._createPreparedEvent(
            event,
            phase,
            preEnd,
            type
